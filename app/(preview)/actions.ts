...
export const generateMatchingTitle = async (file: string) => {
  const result = await generateObject({
    model: google("gemini-1.5-flash-latest"),
    schema: z.object({
      title: z.string().describe("A max three word title for the matching game based on the file provided as context"),
    }),
    prompt:
...
export const generateMatchingPairs = async (content: string) => {
  const result = await generateObject({
    ...
    schema: z
      ...
      .length(6),
    prompt: `Create 6 matching pairs based on this content. Each pair should have:
